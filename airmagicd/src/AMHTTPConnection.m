...
- (id)processURL:(NSString *)path {
    ...
    if ([pathCommand isEqualToString:@"enterText"]) {
        [[AMHelper sharedInstance] enterText:pathValue];
    } else if ([pathCommand isEqualToString:@"appendText"]) {
        [[AMHelper sharedInstance] appendText:pathValue];
    } else if ([pathCommand isEqualToString:@"remoteCommand"]) {
        [[AMHelper sharedInstance] handleNavigationEvent:pathValue];
        ...
        [[AMHelper sharedInstance] sendRebootCommand];
    } else if ([pathCommand isEqualToString:@"relaunch"]) {
        [[AMHelper sharedInstance] sendRespringCommand];
    } else if ([pathCommand isEqualToString:@"kiosk"]) {
        [[AMHelper sharedInstance] launchKioskApp];
    } else if ([pathCommand isEqualToString:@"startScreenSaver"]) {
        [[AMHelper sharedInstance] startScreensaver];
    }
